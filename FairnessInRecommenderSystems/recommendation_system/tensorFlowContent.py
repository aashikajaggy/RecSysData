#from pandas.core.missing import NP_METHODS
import pandas as pd
import numpy as np
...
movielens_zipped_file = keras.utils.get_file(
    "ml-100k(1).zip", movielens_data_file_url, extract=False
)
...
df = pd.read_csv(ratings_file, sep='	', header=None, engine='python', encoding='latin-1')
...
movie_df = pd.read_csv(movielens_dir / "u.item", sep='|', header=None, engine='python', encoding='latin-1')
...
with open('ratingData1.csv', 'w') as file:
    writer = csv.writer(file)
    for i in range(0,99999):
        writer.writerow([df["uIndex"][i], df["mIndex"][i], df["rating"][i], df["timestamp"][i]])
    for x in range(943):
# Let us get a user and see the top recommendations.
        ...
        for row in recommended_movies.itertuples():
            ...
            writer.writerow([user_id, row.mIndex, 5, 888205088])
